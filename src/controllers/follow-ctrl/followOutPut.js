...
const followOutPut = {
  getFollowing: async (req, res) => {
    try {
      ...
      logger.info(`GET /api/followings/${req.params} 200 res:${message}`);
      res.status(200).send({ followingIdList, message });
    } catch (error) {
      // try에서 발생한 오류 catch해서 응답함
      console.error(error);
      message = "알 수 없는 문제가 발생했습니다. 잠시 후 다시 시도해주세요.";
      logger.error(`GET /api/followings/${req.params} 500 res:${error}`);
  ...
  getFollower: async (req, res) => {
    try {
      ...
      logger.error(`GET /api/followers/${req.params} 200 res:${message}`);
      res.status(200).send({ followerIdList, message });
    } catch (error) {
      console.error(error);
      message = "알 수 없는 문제가 발생했습니다. 잠시 후 다시 시도해주세요.";
      logger.error(`GET /api/followers/${req.params} 500 res:${error}`);
      res.status(500).send({ message });
    }
  },
};
