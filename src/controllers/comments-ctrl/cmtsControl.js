...
const comments = {
  ...
  commentCreate: async (req, res) => {
    try {
      ...
      console.log(err);
      return res.status(500).send({ message: "댓글 서버로부터 오류가 생겼습니다." });
    }
  },
  ...
  commentSearch: async (req, res) => {
    try {
      ...
      const commentAll = await Comment.findAll({
        ...
        where: { postId: postId }, // where는 조건절, 결과값에서 postId 컬럼에 조건을 건다. 필터링
      }); // where + include 모두 사용됨
      ...
      console.log(err);
      return res.status(500).send({ message: "댓글 조회로부터 문제가 생겼습니다." });
    }
  },
  ...
  commentDel: async (req, res) => {
    try {
      ...
      const reqDelete = await Comment.findOne({
        ...
        where: { postId, commentId }, // 어디에서 postId 라는 컬럼에서 postId로, id 라는 컬럼에서 commentId로
        // where 옵션으로 나열함으로써, 기본적으로 and 옵션과 같다
      });

      if (reqDelete.userId === userId) {
        // 삭제요청이 들어오면
        await reqDelete.destroy(); // "특정 포스트 -> 특정 댓글" 지운다
        return res.status(200).send({ message: "댓글이 삭제되었습니다." });
      } else {
        return res.status(400).send({ message: "작성자가 아닙니다. " });
      }
    } catch (err) {
